...
import { PagoAdicional } from '../models/pago-adicional';

@Injectable()
export class PagoAdicionalService implements RestService {
    public url: string;
    ...
    constructor(
        ...
        this.url = Config.apiURL + 'pago-adicional';
        this.headers = new HttpHeaders().set('Content-Type', 'application/json');
    }

    save(model: PagoAdicional): Observable<any> {
        let params = JSON.stringify(model);
        return this._http.post(this.url, params, {headers: this.headers});
    }

    update(model: PagoAdicional): Observable<any> {
        let params = JSON.stringify(model);
